...
def compute_det_curve(target_scores, nontarget_scores):
    ...
    frr = np.concatenate((np.atleast_1d(0), tar_trial_sums / target_scores.size))  # false rejection ratesű
    far = np.concatenate((np.atleast_1d(1), nontarget_trial_sums / nontarget_scores.size))  # false acceptance rates
    scores1 = np.atleast_1d(all_scores[indices[0]] - 0.001)
    scores2 = all_scores[indices]

    thresholds = np.concatenate((scores1, scores2))  # Thresholds are the sorted scores
